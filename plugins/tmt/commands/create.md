---
description: Create a business-focused ticket through guided discussion (WHAT & WHY).
...
The ticket focuses on **WHAT** needs to be built and **WHY**, not **HOW** to build it. Technical implementation details are deliberately deferred — they belong to whatever process follows the ticket (with the **tce** plugin: its research and planning phases).
...
Tip: You can also provide an initial description: `/tmt:create Add support for document tagging`
```
...
1. **Determine ticket number** (`[PREFIX]` below is the project's ticket prefix, configured in `.claude/tmt/config` — the script substitutes it automatically):
...
3. **Check downstream consumer expectations**: if the project contains
   `${CLAUDE_PROJECT_DIR}/.claude/tce/tickets.md` with a "What tce needs from a
   ticket" section, read it and make sure the ticket covers what it asks for —
   the tce workflow consumes these tickets. If the file doesn't exist, skip this
   step; the template below stands on its own.
...
6. **Populate every section** based on your discussion:
